            ...
            steps {
                script {
                    sh '''
                        ...
                        curl -v -X POST http://${STAGING_API_ENDPOINT}/staging -H 'Content-Type: application/json'  --data-binary @data.json  2>&1 | grep 200
                    '''
                }
            }
        }
        stage('PROD - Deploy app') {
            ...
            steps {
                script {
                    ...
                    curl -v -X POST http://${PRODUCTION_API_ENDPOINT}/prod -H 'Content-Type: application/json'  --data-binary @data.json  2>&1 | grep 200
                    '''
                }
            }
        }
